...
let jsonObj = [
    ...
    {
      "name": "Sophia",
      "img": "../../assets/images/pets-katкш.png",
      "type": "Dog",
      "breed": "Shih tzu",
...
let navBtn = document.querySelector(".logo__burger-menu");
...
let slogonBtn = document.querySelector("body > main > div > div > div > div > button ");
let ourFriendsBtn = document.querySelector("body > main > div.pets > div > div > button");
let isActiveMenu = true;
let isCreatePopup = true;
let slideArrowRight = document.querySelector("body > main > div.pets > div > div > div > button.slider__button-reverted");
let slideArrowLeft = document.querySelector("body > main > div.pets > div > div > div > button.slider__button");
let petsBlock = document.getElementsByClassName("pets__cards");
...
const activeNav = function() {
    if (isActiveMenu){
      document.body.style.overflowY = "hidden";
        navBtn.classList.add('burger-menu-active');
        document.querySelector("body > header > div > nav.menu").classList.add('menu-active');
        document.querySelector("body > header > div > div.logo").classList.add('logo-active');
        document.querySelector("body > div").style.display = "block";
        isActiveMenu = false;
    } else {
      document.body.style.overflowY = "scroll";
        navBtn.classList.remove('burger-menu-active');
        document.querySelector("body > header > div > nav.menu").classList.remove('menu-active');
        document.querySelector("body > header > div > div.logo").classList.remove('logo-active');
    ...
    if(e.target === slideArrowRight){
      ...
      while(i<3){
          ...
          if(!(countSlide.includes(item)) && !(prevSlide.includes(item))){
            wrapperPets.insertAdjacentHTML('beforeend', `<div class="pets__cards">
            <img src=${item.img}>
              <span class="pets__cards-title">${item.name}</span>
              <button class="pets__cards-button">Learn more</button>
          </div>`);
      ...
      while(i<3){
        item = jsonObj[Math.floor(Math.random()*jsonObj.length)];
        if(!(countSlide.includes(item)) && !(prevSlide.includes(item))){
          wrapperPets.insertAdjacentHTML('afterbegin', `<div class="pets__cards">
          <img src=${item.img}>
            <span class="pets__cards-title">${item.name}</span>
            <button class="pets__cards-button">Learn more</button>
        </div>`);
...
const createPopup = function(event){
  ...
  document.body.style.overflowY = 'hidden';
    if(isCreatePopup){
        nameObj = event.currentTarget.getElementsByClassName('pets__cards-title')[0].innerText;
        itemJsonObj = {};
